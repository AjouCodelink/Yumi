          ...
          (_, { rows: { _array } }) => {
            {
              if (_array.length != 0) {
                ToastAndroid.show(
                  "You already join this room.",
                  ToastAndroid.SHORT
                );
              } else {
                var url =
                  "http://101.101.160.185:3000/chatroom/entrance/" +
                  new_room.cr_id;
                fetch(url, {
                  method: "POST",
                  headers: new Headers({
      ...
      <View style={styles.container}>
            ...
            >
              <Left style={{ justifyContent: "center" }}>
                {item.interest.section == "Foods" ? (
                  <Thumbnail
                    ...
                    source={require("../../assets/cr_thumbnail/foods.jpg")}
                  />
                ) : item.interest.section == "Games" ? (
                  <Thumbnail
                    style={{ width: 50, height: 50, borderRadius: 15 }}
                    source={require("../../assets/cr_thumbnail/games.jpg")}
                  />
                ) : (
                  <Thumbnail
                    style={{ width: 50, height: 50, borderRadius: 15 }}
                    source={require("../../assets/cr_thumbnail/sports.jpg")}
                  />
                )}
                {item.favorite == 1 ? (
                  <Icon
              ...
              <Body>
                ...
                <Text style={{ fontSize: 10, color: "#333" }}>
                  {" "}
                  #{item.interest.section} #{item.interest.group}
                </Text>
                <Text style={{ fontSize: 13, width: "80%", height: 18 }}>
                  {" "}
                  {item.lastMessage != null ? item.lastMessage : "No message"}
                </Text>
              </Body>
              <Right
                style={{
        ...
        <View style={{ width: "100%", height: 80 }}>
          {this.state.suggestedRoom.cr_id == undefined ? (
            <Text style={{ color: "#333", fontSize: 16, padding: 30 }}>
              No room suggested yet.
            </Text>
          ) : (
            <ListItem
              avatar
            ...
            >
              ...
              <Body>
                <Text style={{ fontSize: 16, fontWeight: "bold" }}>
                  {this.state.suggestedRoom.cr_name}
                </Text>
                <Text style={{ fontSize: 10, color: "#333" }}>
                  {" "}
                  #{this.state.suggestedRoom.interest.section} #
                  {this.state.suggestedRoom.interest.group}
                </Text>
                <Text style={{ fontSize: 13 }}>
                  {" "}
                  {this.state.suggestedRoom.lastMessage != null
                    ? item.lastMessage
              ...
              >
                <Icon name="md-people" style={{ fontSize: 16, color: "#333" }}>
                  <Text style={{ fontSize: 14, color: "#333" }}>
                    {" "}
                    {this.state.suggestedRoom.memNum}
                  </Text>
                </Icon>
              </Right>
            </ListItem>
